...
func _ready() -> void:
	...
	# Ensure tutorial scene music players have correct streams
	var tutorial_scene = get_tree().get_root().find_child("Tutorial", true, false)
	if tutorial_scene:
		var first_music = tutorial_scene.find_child("firstMusic", true, false)
		var battle_music = tutorial_scene.find_child("battleMusic", true, false)
		if first_music:
			first_music.stream = load("res://audio/Music/travels_bgm story mode.wav")
		if battle_music:
			battle_music.stream = load("res://audio/Music/dad_battle.wav")
...
func action() -> void:
	...
	# Start dialogue music and stop battle music
	var tutorial_scene = get_tree().get_root().find_child("Tutorial", true, false)
	if tutorial_scene:
		var first_music = tutorial_scene.find_child("firstMusic", true, false)
		var battle_music = tutorial_scene.find_child("battleMusic", true, false)
		if first_music and not first_music.playing:
			first_music.play()
