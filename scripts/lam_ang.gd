...
const BASE_ATTACK_DAMAGE = 20.0  # ADD THIS LINE HERE!
...
func attack():
	...
	if Input.is_action_just_pressed("attack") and not isPassiveCD:
		...
		# ✅ Keep AttackArea always monitoring
		for body in attackArea.get_overlapping_bodies():
			print("Overlapping:", body.name)
			if isAttacking and body.has_method("deal_dmg"):
				print("Dealing damage to:", body.name)
				body.deal_dmg(BASE_ATTACK_DAMAGE + strengthBuff)
...
func _on_attack_area_body_entered(body: Node2D) -> void:
	if not body or not is_instance_valid(body):
		return
	if not Global.playerCurrentAttack:
		return
	
	var target = body
	if not body.has_method("deal_dmg") and body.get_parent():
		target = body.get_parent()
	
	if target.has_method("deal_dmg"):
		target.deal_dmg(BASE_ATTACK_DAMAGE + strengthBuff)
